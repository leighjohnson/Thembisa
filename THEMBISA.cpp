...
void UpdatePrEPandVM()
{
	...
	// CAB-LA initiation (preliminary, assuming no CAB-LA included in previously reported numbers)
	if (CurrYear <= PrEPdataYr){ CurrCABLArateFSW = 0.0; }
	else{ CurrCABLArateFSW = UltCABLArateFSW; }
	...
	for (ia = 0; ia < 81; ia++){
		for (ii = 0; ii < 2; ii++){
			CABLArates[ia][ii] = CurrCABLArateFSW * RR_PrEPstartMSM[iy] * RR_PrEP_MSM[ia][ii];
			CABLArates[ia][ii + 2] = CurrCABLArateFSW * RR_PrEPstartF20[iy] * RR_PrEP_Het[ia][ii][0];
			CABLArates[ia][4] = CurrCABLArateFSW;
			CABLArates[ia][ii + 5] = CurrCABLArateFSW * RR_PrEPstartF20[iy] * RR_PrEP_Het[ia][ii][1];
		}
	}
	// Pregnancy adjustment in women
...
void ResultsAtStartOfYr()
{
	...
	if (FixedUncertainty == 1){
		...
		for (ia = 0; ia < 81; ia++){
			...
			Temp4 += SumGroupsM[ia][2] + SumGroupsF[ia][2];
		...
		TotOnCABLA.out[CurrSim - 1][iy] = Temp4;
		Temp1 = 0.0;
	...
	if (CurrYear == StartYear){
		...
		for (is = 0; is < 4; is++){
			...
			for (id = 1; id < 5; id++){
				...
				DisabilityWeights[15 + is * 5 + id] = IHMEdisab[4] * OnARThalfIntDur[id][0][25];
			}
		}
		for (is = 0; is < 4; is++){
...
void GetPrEPrateFSW()
{
	...
	if (FixedUncertainty == 1){ 
		...
		if (temp > 0.0){ PrEPcoverageAGYW.out[CurrSim - 1][iy] = AGYWonPrEP.out[CurrSim - 1][iy] / (temp + 
			AGYWonPrEP.out[CurrSim - 1][iy]); }
		for (ia = 15; ia < 81; ia++){ temp += PrEPeligOther[ia][0][1] + PrEPeligOther[ia][1][1]; }
		if (PrEPeligFSW > 0.0){ PrEPcoverageAllF.out[CurrSim - 1][iy] = WomenOnPrEP.out[CurrSim - 1][iy] / (temp + 
			PrEPeligFSW + WomenOnPrEP.out[CurrSim - 1][iy]);}
		temp2 = 0.0;
		for (ia = 0; ia < 81; ia++){ temp2 += PrEPeligOther[ia][0][0] + PrEPeligOther[ia][1][0]; }
		if (tempMSM > 0.0){ PrEPcoverageAllM.out[CurrSim - 1][iy] = MenOnPrEP.out[CurrSim - 1][iy] / (temp2 + 
			tempMSM + MenOnPrEP.out[CurrSim - 1][iy]);}
		if (PrEPeligFSW > 0.0){
			PrEPcoverageAll.out[CurrSim - 1][iy] = (WomenOnPrEP.out[CurrSim - 1][iy] + MenOnPrEP.out[CurrSim - 1][iy]) /
				(temp + PrEPeligFSW + temp2 + tempMSM + WomenOnPrEP.out[CurrSim - 1][iy] + MenOnPrEP.out[CurrSim - 1][iy]);
		}
	}
}
...
{
	double temp, temp2, MaxMinDif1, MaxMinDif2, a_term, b_term;
	int ic, ia, im, iy;
...
void ResultsAtEndOfYr()
{
	...
	if (CalibHCTprev == 1 && CurrYear >=2004){
		...
		if (CurrYear >= 2015 && CurrYear<2023){ ModelPrevHCT[iy-23] = Temp[0]; }
	}
...
void GetSummaryOutputs(char* filout)
{
	...
	NewHIVclientsNotSW.GetMeans(); // New in version 4.7
	...
	HIVinc50F.GetMeans();
	HIVincFSW.GetMeans(); 
	HIVincClients.GetMeans(); // New in version 4.7
	HIVincMSM.GetMeans(); 
	ANCincidence.GetMeans(); // New in version 4.7
	IncPrevRatio.GetMeans(); 
	IncPrevRatioFtoM.GetMeans(); 
	IncPrevRatioMtoF.GetMeans(); 
	NewMTCT.GetMeans();
	NewHIVatBirth.GetMeans();
	NewHIVatBirthART.GetMeans(); // New in version 4.7
	NewHIVatBirthNoKOS.GetMeans(); // New in version 4.7
	NewHIVafterBirth.GetMeans();
	NewHIVafterBirthART.GetMeans(); // New in version 4.7
	NewHIVafterBirthNoKOS.GetMeans(); // New in version 4.7
	...
	PrevClients.GetMeans(); // New to Thembisa 4.7
	...
	NewARTpaedAdvanced.GetMeans(); // New to Thembisa 4.7
	TotalOnART.GetMeans();
	...
	ARTcoverageClients.GetMeans(); // New to Thembisa 4.7
	...
	DiscontinuedART_M.GetMeans(); // New to Thembisa 4.7
	DiscontinuedART_F.GetMeans(); // New to Thembisa 4.7
	ChildARTinterrupters.GetMeans(); // New to Thembisa 4.7

	// VL suppression outputs (mostly new to v4.7)
	SummOutRow += 3;
	VLsuppressed.GetMeans();
	VLsuppressedM.GetMeans();
	VLsuppressedF.GetMeans();
...
{
	int iy;
	double HCTlogL, ModelError, VarLogit[15];
...
double CalcARTcoverageLogL()
{
	...
	// 2022 survey
	LogitDif = log(ModelCoverage[0][2] / (1.0 - ModelCoverage[0][1])) -
